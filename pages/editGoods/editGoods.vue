<template>
	<view class="fb_page">
		...
		<button class="submitBtn" @click="updateGoods">保存</button>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			if (option != '') {
				console.log(option)
				this.loadGoodsData(option.goodsId);
			}
			this.loadSortData();
				...
				this.$post('goods/selectByGoodsId', {
					...
					if (res.code == 200) {
						if (res.result == 'success') {
							...
							this.description = res.data.goods.description;
							this.chooseImages = [];
							res.data.imgList.forEach(v => {
								this.chooseImages.push(v.goodsImg);
							});
							this.sortIndex = res.data.goods.sort;
							this.nowPrice = res.data.goods.nowPrice;
							this.oldPrice = res.data.goods.oldPrice;
							this.label = res.data.goods.label;
							this.grade = res.data.goods.grade;
							this.rankArrays.forEach((v, i) => {

								if (v === res.data.goods.grade) {
									this.rankIndex = i;

								}
							});
							this.labelArrays.forEach((v, i) => {
								if (v === res.data.goods.label) {
									this.labelIndex = i;
								}
							})
